@if(\Auth::check() && \Auth::user()->canManageBlogEtcPosts())
    <a href="{{$post->edit_url()}}" class="btn btn-outline-secondary btn-sm pull-right float-right">Éditer le post</a>
@endif
...
Posté il y a <strong>{{substr($post->posted_at->diffForHumans(), 0, strlen($post->posted_at->diffForHumans())-4)}}</strong>
